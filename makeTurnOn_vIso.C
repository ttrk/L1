...
//    double sigmaietaieta = -1;
    ...
    fPt_iso[0]->Fill(maxfpt_iso);
    fPt_sideBand[0]->Fill(maxfpt_sideBand);
    if(hiBin < 60){
        fPt_iso[1]->Fill(maxfpt_iso);
        fPt_sideBand[1]->Fill(maxfpt_sideBand);
    }
    else if (hiBin >= 100) {
        fPt_iso[2]->Fill(maxfpt_iso);
        fPt_sideBand[2]->Fill(maxfpt_sideBand);
    }

    if(maxfpt_iso > 40)
    {
      fPtSigma_iso[0]->Fill(sigmaietaieta_iso);
      if(hiBin < 60){
          fPtSigma_iso[1]->Fill(sigmaietaieta_iso);
      }
      else if (hiBin >= 100) {
          fPtSigma_iso[2]->Fill(sigmaietaieta_iso);
      }
    }

    if(maxfpt_sideBand > 40)
    {
      fPtSigma_sideBand[0]->Fill(sigmaietaieta_sideBand);
      if(hiBin < 60){
          fPtSigma_sideBand[1]->Fill(sigmaietaieta_sideBand);
      }
      else if (hiBin >= 100) {
          fPtSigma_sideBand[2]->Fill(sigmaietaieta_sideBand);
      }
    }
    ...
    for(int i = 0; i < THRESHOLDS; ++i)
    {
      ...
      {
	accepted_iso[i][0]->Fill(maxfpt_iso);
	accepted_sideBand[i][0]->Fill(maxfpt_sideBand);
	if(hiBin < 60){
	    accepted_iso[i][1]->Fill(maxfpt_iso);
	    accepted_sideBand[i][1]->Fill(maxfpt_sideBand);
	}
	else if (hiBin >= 100){
	    accepted_iso[i][2]->Fill(maxfpt_iso);
	    accepted_sideBand[i][2]->Fill(maxfpt_sideBand);
	}

	if(maxfpt_iso > 40)
	{
	  accepted_sieie_iso[i][0]->Fill(sigmaietaieta_iso);
	  if(hiBin < 60){
	      accepted_sieie_iso[i][1]->Fill(sigmaietaieta_iso);
	  }
	  else if (hiBin >= 100){
	      accepted_sieie_iso[i][2]->Fill(sigmaietaieta_iso);
	  }
	}

    if(maxfpt_sideBand > 40)
    {
      accepted_sieie_sideBand[i][0]->Fill(sigmaietaieta_sideBand);
      if(hiBin < 60){
          accepted_sieie_sideBand[i][1]->Fill(sigmaietaieta_sideBand);
      }
      else if (hiBin >= 100){
          accepted_sieie_sideBand[i][2]->Fill(sigmaietaieta_sideBand);
      }
    }
...
//      a_sieie[k][l]->SetName(Form("asymm_sieie_%d_%d",k,l));
      ...
      a_iso[k][l]->SetTitle("pT turn on - isolated");
      ...
      a_sieie_iso[k][l]->SetTitle("sigmaIEtaEta turn on - isolated");
      ...
      a_iso[k][l]->SetTitle("pT turn on - sideband");
      ...
      a_sieie_sideBand[k][l]->SetTitle("sigmaIEtaEta turn on - sideband");
    }
  }
